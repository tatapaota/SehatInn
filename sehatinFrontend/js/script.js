document.addEventListener("DOMContentLoaded", function () {
  ...
  // ==========================
  // 🔧 PENYESUAIAN AGAR DATA ANTAR HALAMAN NYAMBUNG
  // ==========================
  const AppStorage = {
    set(key, value) {
      try {
        // gunakan localStorage biar gak hilang pas pindah halaman
  ...
  // Tambahan opsional: auto handle tombol logout
  ...
  if (registerForm) {
    registerForm.addEventListener("submit", function (e) {
      ...
      // Kirim data ke server (auth_register.php)
  ...
  // HOME PAGE (Home.html) - DENGAN DOSIS OBAT
  if (document.body.classList.contains("home-page")) {
    ...
    function createScheduleItemWithMeds(
      ...
      newSchedule.innerHTML = `
        ...
        border-radius: 8px; font-size: 14px;"><option value="" disabled selected hidden>Pilih waktu minum obat</option>
        <option value="Sebelum makan">Sebelum makan</option><option value="Setelah makan">Setelah makan</option>
        <option value="Waktu lain">Waktu lain</option></select>
    ...
    function saveScheduleData() {
      ...
      // HAPUS BARIS INI (const date = dateInput.value;) KARENA VARIABLE 'date' SUDAH ADA DI ATAS

      if (currentUser) {
        schedules.forEach((s) => {
          if (s.medications) {
            s.medications.forEach((med) => {
              // Gunakan variabel 'date' yang sudah didefinisikan di awal fungsi
...
async function saveReminder(user_id, med_name, dosage, time, date) {
  try {
    ...
    const data = await res.json();
    console.log("Status Simpan DB:", data);
  } catch (error) {
...
function togglePasswordVisibility() {
  ...
  if (passwordInput.type === "password") {
    ...
    toggleIcon.src = "aset/eye-open.png";
    toggleIcon.alt = "hide password";
  } else {
    passwordInput.type = "password";
    toggleIcon.src = "aset/eye-closed.png";
